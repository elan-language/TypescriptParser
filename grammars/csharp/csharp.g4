...
main: 
    STATIC VOID MAIN OPEN_BRACKET CLOSE_BRACKET OPEN_BRACE NL
       ordinaryStatement*
...
test: 
    TEST_CLASS_ANNOT CLASS typeName NL
      TEST_METHOD_ANNOT STATIC VOID testName
        (assert | letStatement | variableDefinition | commentLine)*
    CLOSE_BRACE COMMENT NL
    ;
...
concreteClass:
    CLASS typeName (COLON typeName)? OPEN_BRACE NL
        (constructorMember | property | functionMethod | procedureMethod | commentLine)*
    CLOSE_BRACE COMMENT NL;

abstractClass:
    ABSTRACT CLASS typeName (COLON typeName)? OPEN_BRACE NL
        (property | functionMethod | procedureMethod | abstractFunction | abstractProcedure | commentLine)*
    CLOSE_BRACE COMMENT NL;
...
whileLoop:
    WHILE OPEN_BRACKET expression CLOSE_BRACKET OPEN_BRACE NL
       ordinaryStatement*
    CLOSE_BRACE COMMENT NL
    ;
...
forLoop:
    FOREACH OPEN_BRACKET VAR identifier IN expression CLOSE_BRACKET OPEN_BRACE NL
       ordinaryStatement*
    CLOSE_BRACE COMMENT NL
    ;
...
assert: ASSERT DOT ARE_EQUAL OPEN_BRACKET assertActual COMMA expression CLOSE_BRACKET SEMI_COLON NL; 
letStatement: VAR identifier SINGLE_EQUALS expression SEMI_COLON COMMENT NL;
print: PRINT OPEN_BRACKET expression? CLOSE_BRACKET  SEMI_COLON NL;
variableDefinition: VAR identifier SINGLE_EQUALS expression SEMI_COLON NL; 
assignment: assignable SINGLE_EQUALS expression  SEMI_COLON COMMENT NL; 
inputStatement: identifier EQUAL INPUT OPEN_BRACKET expression CLOSE_BRACKET  SEMI_COLON NL; 
procedureCall: term  SEMI_COLON COMMENT NL; // Compiler to check that term ends in a methodCall, and that the method is a procedure
throwStatement: THROW NEW typeName OPEN_BRACKET expression CLOSE_BRACKET  SEMI_COLON NL;
returnStatement: RETURN expression SEMI_COLON NL;
elseIfClause: CLOSE_BRACE ELSE IF OPEN_BRACKET expression CLOSE_BRACKET OPEN_BRACE NL;
elseClause: CLOSE_BRACE ELSE OPEN_BRACE NL;
catchStatement: CLOSE_BRACE CATCH OPEN_BRACKET typeName identifier CLOSE_BRACKET OPEN_BRACE NL;

// Members
constructorMember:
    PUBLIC OPEN_BRACKET paramsList? CLOSE_BRACKET OPEN_BRACE NL
        ordinaryStatement*
    CLOSE_BRACE COMMENT NL
    ;
...
abstractFunction: ABSTRACT type methodName OPEN_BRACKET paramsList? CLOSE_BRACKET SEMI_COLON COMMENT NL;


abstractProcedure: ABSTRACT VOID methodName OPEN_BRACKET paramsList? CLOSE_BRACKET SEMI_COLON COMMENT NL;
